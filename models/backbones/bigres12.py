...
class ResNet(nn.Module):
    ...
    def __init__(self, block=BasicBlock, num_classes=1000,remove_linear=True, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=5):
        self.inplanes = 6
        ...
        # self.layer1 = self._make_layer(block, 64, stride=2, drop_rate=drop_rate)
        # self.layer2 = self._make_layer(block, 160, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        # self.layer3 = self._make_layer(block, 320, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        # self.layer4 = self._make_layer(block, 640, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        if avg_pool:
    ...
    def fft_input(self, X):
        ...
        # 截断频率分量，假设截断比例为truncate_ratio
        truncate_ratio = 0.70713  # 保留中心50%的频率分量
        C, T, H, W = X.shape
        ...
        mask = mask.to(X_fft.device) # 将mask转移到与X_fft相同的设备上
        X_fft = X_fft * mask
        
        # 对FFT变换后的张量进行逆变换得到频率域的图像表示
...
def bigres12(keep_prob=1.0, avg_pool=False, **kwargs):
    """Constructs a ResNet-12 model.
    """
    model = ResNet(BasicBlock, keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    return model

def bigres12_head(keep_prob=1.0, avg_pool=False, **kwargs):
    """Constructs a ResNet-12 model.
    """
    model = ResNet_head(BasicBlock, keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    return model
